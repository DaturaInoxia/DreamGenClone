"""Render a single-person OpenPose JSON pose to a 1024x1024 skeleton PNG
for ControlNet conditioning. Reuses the drawing conventions from
merge-openpose-pair.py but does not compose a second person.

Usage:
  python helpers/runpod/render-single-pose.py <pose.json> <out.png>
"""
import json
import sys
...
def main():
    src = Path(sys.argv[1])
    out = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("pose.png")
    person = fit(load_person(src))
    saved = render(person, out)
    print(f"Rendered {src.name} -> {saved}")
