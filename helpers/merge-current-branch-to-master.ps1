<#
.SYNOPSIS
Merges the current branch into master.
...
3) checks out master,
4) optionally fast-forwards master from origin,
5) merges source branch into master with --no-ff,
6) optionally pushes master to origin.

.EXAMPLE
./helpers/merge-current-branch-to-master.ps1

.EXAMPLE
./helpers/merge-current-branch-to-master.ps1 -Push

.EXAMPLE
./helpers/merge-current-branch-to-master.ps1 -SourceBranch feature/my-work -Push
#>
...
if ($SourceBranch -eq "master") {
    throw "Source branch resolves to 'master'. Checkout your feature branch first or pass -SourceBranch."
}
...
& git -C $RepositoryPath show-ref --verify --quiet "refs/heads/master"
if ($LASTEXITCODE -ne 0) {
    throw "Local branch 'master' does not exist."
}

Write-Section "Merging '$SourceBranch' into master"

if ($currentBranch -ne "master") {
    Write-Host "Checking out master..." -ForegroundColor Yellow
...
if (-not $SkipPull) {
    if (Test-OriginRemote) {
        Write-Host "Pulling latest origin/master (ff-only)..." -ForegroundColor Yellow
        Invoke-Git -Args @("pull", "--ff-only", "origin", "master")
    }
    else {
...
if (-not $Push) {
    Write-Host "Run 'git push origin master' when ready." -ForegroundColor Cyan
}