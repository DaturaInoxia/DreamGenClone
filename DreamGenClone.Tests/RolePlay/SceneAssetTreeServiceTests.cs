...
public sealed class SceneAssetTreeServiceTests
{
    ...
    public async Task BuildTreeAsync_CharacterWithPacks_IsAStudioEntry()
    {
        ...
        {
            Name = "Test",
            Characters = [new Character { Id = "char-1", Name = "Dean" }]
        };
        var packs = new Dictionary<string, CharacterImageIdentityPack>
        {
            ["pack-v2"] = new() { Id = "pack-v2", CharacterProfileId = "char-1", Version = 2, Status = CharacterImageIdentityPackStatus.Draft },
            ["pack-v1"] = new() { Id = "pack-v1", CharacterProfileId = "char-1", Version = 1, Status = CharacterImageIdentityPackStatus.Approved }
        };
        ...
        Assert.Equal("/characters/char-1", root.Owner.Href);
        Assert.Empty(root.Groups);
    }

    [Fact]
    public async Task BuildTreeAsync_MergesSameNamedCharactersAcrossScenarios()
    {
        var scenarioA = new Scenario
        {
            Name = "Campground",
            Characters = [new Character { Id = "dean-a", Name = "Dean" }]
        };
        var scenarioB = new Scenario
        {
            Name = "The Party",
            Characters = [new Character { Id = "dean-b", Name = "Dean" }]
        };
        var packs = new Dictionary<string, CharacterImageIdentityPack>
        {
            ["pack-a"] = new() { Id = "pack-a", CharacterProfileId = "dean-a", Version = 1, Status = CharacterImageIdentityPackStatus.Draft }
        };
        ...
        Assert.Equal("Character", root.Owner.RootKind);
        Assert.Equal("Dean", root.Owner.OwnerName);
        Assert.Equal("/characters/dean-a", root.Owner.Href);
    }
        ...
        IReadOnlyDictionary<string, SceneImageReferenceAsset> assets) : ICharacterImageIdentityService
    {
        public Task<IReadOnlyList<CharacterImageIdentityPack>> ListPacksAsync(string characterProfileId, CancellationToken cancellationToken = default)
            => Task.FromResult<IReadOnlyList<CharacterImageIdentityPack>>(
                packs.Values.Where(p => p.CharacterProfileId == characterProfileId).OrderByDescending(p => p.Version).ToList());
        ...
        public Task<CharacterImageIdentityPack> CreateDraftPackAsync(string characterProfileId, CancellationToken cancellationToken = default) => throw new NotSupportedException();
