...
public sealed class GenericBackgroundJobQueue : IBackgroundJobQueue
{
    ...
    private readonly ILogger<GenericBackgroundJobQueue> _logger;

    public GenericBackgroundJobQueue(ILogger<GenericBackgroundJobQueue> logger)
    {
        _logger = logger;
    }
    ...
    public bool Enqueue(string jobType, string payloadJson, string? dedupeKey = null)
    {
        ...
        {
            _logger.LogDebug("Skipped duplicate background job enqueue: {JobType} {DedupeKey}", normalizedJobType, normalizedDedupeKey);
            return false;
        }
    ...
    public void MarkFailed(string jobId, string errorMessage)
    {
        ...
        _logger.LogWarning("Background job {JobId} failed: {ErrorMessage}", jobId, errorMessage);
    }
