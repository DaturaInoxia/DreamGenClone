...
target_id = 'c8667fef-3f49-409a-bb30-23228a999a27'
...
cname = f'response_{short_iid}_[{target_idx}]_{safe_actor}.txt'
cpath = os.path.join(outdir, cname)
with open(cpath, 'w', encoding='utf-8') as f:
    f.write(content)
print(f'Saved response to: {cpath}')
...
# Show context: surrounding interactions
print('\n--- SURROUNDING INTERACTIONS ---')
for i in range(max(0, target_idx-3), min(len(ixns), target_idx+4)):
    ix = ixns[i]
    ix_id = ix.get('id', '?')[:8]
    actor = ix.get('actorName', '?')
    itype = ix.get('interactionType', '?')
    clen = len(ix.get('content', ''))
    marker = ' <<<' if i == target_idx else ''
    print(f'  [{i}] {ix_id} | {actor:10s} | {itype} | {clen}c{marker}')
