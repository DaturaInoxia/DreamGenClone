#!/usr/bin/env python3
"""
Two-character identity matrix runner (PLUS FACE regional, single frontal refs).

Part of the `identity-two-character` test suite
(specs/image-generator-tests/identity-two-character/). Each cell conditions a
1024x1024 Juggernaut render with per-character IP-Adapter "PLUS FACE (portraits)"
regional `attn_mask`. Dean is always left, Becky right.

Fully parameterized so the same suite can be re-run against any pod / character
set. Defaults resolve to the suite's own inputs and an ignored artifacts/tmp output.

Usage:
  python run_matrix.py --cell c2 --seed 1001 --strength 0.8
  python run_matrix.py --all --seeds 1001 1002          # run 6 cells x seeds
  python run_matrix.py --cell c1 --seed 1001 --dump-json  # print exact workflow JSON
"""
...
DEFAULT_OUT = os.path.join(os.environ.get("DG_TMP", r"d:\src\DreamGenClone\artifacts\tmp"),
                           "two-character-proof", "outputs")
DEFAULT_MASKS = os.path.join(SUITE, "masks")
DEFAULT_REF_DIR = os.path.join(SUITE, "refs")  # dean_face.png / becky_face.jpg
    ...
    return {
        ...
        "20": {"class_type": "IPAdapter",
               "inputs": {"model": ["10", 0], "ipadapter": ["10", 1], "image": ["11", 0],
                          "weight": strength, "weight_type": "standard", "start_at": 0.0, "end_at": 1.0,
                          "attn_mask": ["13", 0]}},
        ...
        "9": {"class_type": "SaveImage", "inputs": {"filename_prefix": f"two_char_{cell}", "images": ["8", 0]}},
    }
...
def run_one(base, cell, seed, dean_ref, becky_ref, masks_dir, out_dir, strength):
    ...
    payload = {"prompt": workflow, "client_id": f"two-char-{cell}-{seed}"}
...
def main():
    ...
    ap.add_argument("--dump-json", action="store_true", help="print exact workflow JSON for the case and exit")
    args = ap.parse_args()
