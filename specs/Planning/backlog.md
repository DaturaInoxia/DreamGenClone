...
| B-021 | Character stats UI — show baseline and phase-unlock requirements | `new` | In the character stats view, display (1) the starting baseline values from the assigned stat profile so the user can see where stats began, and (2) which stats need to reach which thresholds to unblock the next phase transition — giving the user clear visibility into what must change to advance the narrative |
